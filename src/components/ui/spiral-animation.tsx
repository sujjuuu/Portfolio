...
class Star {
  ...
  render(p: number, c: AnimationController) {
    ...
    const displacementProgress = c.constrain(4 * q, 0, 1)
    const linearEasing = displacementProgress
    const elasticEasing = c.easeOutElastic(displacementProgress)
    const powerEasing = Math.pow(displacementProgress, 2)

    let easing: number
    if (displacementProgress < 0.3) {
      easing = c.lerp(linearEasing, powerEasing, displacementProgress / 0.3)
    } else if (displacementProgress < 0.7) {
      const t = (displacementProgress - 0.3) / 0.4
      easing = c.lerp(powerEasing, elasticEasing, t)
    } else {
